...
int getHighByte(int charCode) {
  // Hack! Convert chars from 0x80 to 0xBF in cyrillic symbols.
  int highByte = 0x0;

  if (charCode >= 0x90 && charCode <= 0xBF) {
    highByte = 0xD0;
  }

  if (charCode >= 0x80 && charCode <= 0x8F) {
    highByte = 0xD1;
  }
...
int utf8BytesToDecimal(int highByte, int lowByte) {
  int high = (highByte & 0b00111111) << 8;
  int low = lowByte & 0b00011111;

  return (high >> 2) + low;
}

void printUtf8Char(int charCode) {
  int highByte = getHighByte(charCode);
  char utf8[3] = { charCode, 0x00, 0x00 };
...
int main(int argc, char *argv[]) {
  int i, j, err, size, first = 0x20, last = 0xBF, bitmapOffset = 0, x, y, byte;
  char *fontName, c, *ptr;
  FT_Library library;
  FT_Face face;
  FT_Glyph glyph;
  ...
  // Default first and last chars are
  // ' ' (space) and '~', and 0x80 - 0xBF reserved for cyrillic chars.
  ...
  for (i = first, j = 0; i <= last; i++, j++) {
    ...
    int charCode = i, highByte = getHighByte(charCode);

    if (highByte) {
      charCode = utf8BytesToDecimal(highByte, charCode);
    }
